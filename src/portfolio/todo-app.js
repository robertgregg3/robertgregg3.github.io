...
const todoCategoryContainer = document.getElementById(
  "todo-list-categories-ul"
);
...
const createListPopupBtn = createListPopup.querySelector(
  "#create-list__button"
);
...
mobileListBtn.addEventListener("click", () => {
  ...
  todoCategoriesCont.classList[
    todoCategoriesCont.classList.contains("toggle-list-container-on-mobile")
      ? "remove"
      : "add"
  ]("toggle-list-container-on-mobile");
  mobileListHeaderIcon.classList[
    mobileListHeaderIcon.classList.contains("hidden") ? "remove" : "add"
  ]("hidden");
  mobileListHeaderIcon2.classList[
    mobileListHeaderIcon2.classList.contains("hidden") ? "remove" : "add"
  ]("hidden");
  todoColumn.classList[
    todoColumn.classList.contains("mobile-btn-hidden") ? "remove" : "add"
  ]("mobile-btn-hidden");
  mobileBtnInactive.classList[
    mobileBtnInactive.classList.contains("mobile-btn-hidden") ? "remove" : "add"
  ]("mobile-btn-hidden");
  mobileBtnActive.classList[
    mobileBtnActive.classList.contains("mobile-btn-hidden") ? "remove" : "add"
  ]("mobile-btn-hidden");
});
...
function createAccPreviewFile() {
  ...
  const createAccfile =
    createAccContainer.querySelector("input[type=file]").files[0];
  ...
  if (createAccfile) reader.readAsDataURL(createAccfile);
  updateLS();
}
...
function showCreateAccountScreen() {
  ...
  const createFirstListBtn = document.querySelector(
    ".create-list-popup-initial #create-list__button"
  );
...
function createList(todoCategoryName) {
  ...
  if (todoCategoryName) {
    createListEl.className +=
      " " + todoCategoryName.split(" ").join("-").toLowerCase();
    ...
    categoryText.addEventListener("keypress", (e) => {
      if (e.code === "Enter" || e.keyCode === 13) e.preventDefault();
    });
    ...
    saveCategoryBtn.addEventListener("click", () => {
      ...
      selectedCategory = createListEl.innerText
        .split(" ")
        .join("-")
        .toLowerCase();
    ...
    function showDeleteCategoryPopup() {
      ...
      const deleteCategoryPopupBtn = deleteCategoryPopup.querySelector(
        ".delete-list__button"
      );
      ...
      const closeDeleteListPopupBtn =
        deleteCategoryPopup.querySelector(".close-delete-list");
    ...
    createListEl.addEventListener("click", (e) => {
      ...
      if (
        todoCategoriesCont.classList.contains(
          "toggle-list-container-on-mobile"
        ) &&
      ...
      todoCategoryName = e.currentTarget.innerText
        .split(" ")
        .join("-")
        .toLowerCase();
    });
...
function filterTodosWhenClicked() {
  ...
  allCategories.forEach((cat) => {
    cat.addEventListener("click", (e) => {
      clickedItemEl = e.currentTarget.innerText
        .split(" ")
        .join("-")
        .toLowerCase();
...
function addTodo(el, todoCategoryName) {
  ...
  if (todoText) {
    ...
    todoItem.addEventListener("mouseover", () => {
      closeTodoBtn.classList.remove("close-hidden");
    });
    todoItem.addEventListener("mouseout", () => {
      closeTodoBtn.classList.add("close-hidden");
    });
    ...
    expandTodoBtn.addEventListener("click", () => {
      todoItem.classList[
        todoItem.classList.contains("todo-item-height") ? "remove" : "add"
      ]("todo-item-height");
      todoItem.classList[
        todoItem.classList.contains("todo-item-tilt") ? "remove" : "add"
      ]("todo-item-tilt");
      todoExtendingDiv.classList[
        todoExtendingDiv.classList.contains("height-100") ? "remove" : "add"
      ]("height-100");
      expandTodoBtn.classList[
        expandTodoBtn.classList.contains("expand-todo--rotate")
          ? "remove"
          : "add"
      ]("expand-todo--rotate");
    });
    ...
    if (el && el.subTasks)
      for (let i = 0; i < el.subTasks.length; i++) createSubtask(i);
    ...
    function createSubtask(i) {
      ...
      if (el && el.subTasks && !subtaskInputClicked) {
        // if there is a todo, and subtasks, and the enter button was NOT pressed
      ...
      subtaskCheckbox.addEventListener("click", () => {
        subtaskOutput.classList[
          subtaskOutput.classList.contains("completed") ? "remove" : "add"
        ]("completed");
        updateLS();
      });
      ...
      subtaskFavoriteBtn.addEventListener("click", () => {
        subtaskFavoriteBtn.classList[
          subtaskFavoriteBtn.classList.contains("favorited") ? "remove" : "add"
        ]("favorited");
        updateLS();
      });
    ...
    todoItemCheckbox.addEventListener("click", () => {
      todoInputText.classList[
        todoInputText.classList.contains("completed") ? "remove" : "add"
      ]("completed");
      todoItem.classList[
        todoItem.classList.contains("hidden") ? "remove" : "add"
      ]("hidden");
      updateLS();
      countTodos();
    });
    ...
    // text area note
    if (el && el.todoNote) todoNote.value = el.todoNote;
...
function updateLS() {
  ...
  localStorage.setItem(
    "accountCreated",
    initialScreensBgLS.classList.contains("initial-hidden")
  );
  ...
  todosEls.forEach((todoEl) => {
    ...
    categories.forEach((category) => {
      if (
        todoEl.classList.contains(
          category.innerText.split(" ").join("-").toLowerCase()
        )
      )
...
function toolbarButtons() {
  ...
  const todoItemCategories = document.querySelectorAll(
    ".todo-list-category-li"
  );
  const totalTodoItems = document.querySelectorAll(".todo-item");
  ...
  showRemainingBtn.addEventListener("click", () => {
    totalTodos.forEach((remainingTodo) => {
      remainingTodo.parentNode.classList[
        remainingTodo.classList.contains("completed") ? "add" : "remove"
      ]("hidden");
    });
  });
  ...
  showCompletedBtn.addEventListener("click", () => {
    totalTodos.forEach((completedTodo) => {
      completedTodo.parentNode.classList[
        completedTodo.classList.contains("completed") ? "remove" : "add"
      ]("hidden");
    });
  });
...
function reorderItems() {
  ...
  const container = document.querySelector(
    "#todos-ul:not(#todo-list-categories-ul)"
  );
...
function reorderCategoryLists() {
  ...
  const container = document.querySelector(
    "#todo-list-categories-ul:not(#todos-ul)"
  );
...
function draggingItemMobile(container) {
  container.addEventListener("touchmove", (e) => {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
      ...
      const afterElement = getDragListAfterElement(
        draggableElements,
...
function draggingList(container) {
  container.addEventListener("dragover", (e) => {
    e.preventDefault();
    const draggableElements = [
      ...document.querySelectorAll(".draggable-list:not(.dragging-list)"),
    ];
    const afterElement = getDragListAfterElement(draggableElements, e.clientY);
    placeDraggingElement(afterElement, container);
  });
